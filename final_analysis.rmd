...
plot_auc <- function(data, title = " ", comparisons, adjust=TRUE) {
  ttest2 <- function(a, b) {
    d <- c(a,b)
  ...
  if(length(levels(data$variable)) > 1) {
    print("Permuted independence test (coin package):")
    tr.coin <- coin::independence_test(class + variable ~ value, 
                                 data=data, teststat="quadratic", 
                                 distribution=approximate(nresample = 10000), 
                                 trafo(data, numeric_trafo = rank_trafo))
    print(tr.coin)
    print("Permuted Wald Type Test (GFD package):")
    tr.gfd <- GFD::GFD(value ~ class + variable, data = data)
    print(summary(tr.gfd))
  } else {
    print("Permutation based Kruskal Wallis Test p-value:")
    tr.coin <- coin::kruskal_test(value ~ class, 
                             data=data, 
                             distribution=approximate(nresample = 10000))
    print(tr.coin)
    print("Permuted Wald Type Test (GFD package):")
...
auc_mccaroll <- function() {
  ...
  plot_auc(dat, comparisons = comparisons) +
    scale_color_discrete(labels=c(expression(paste("G1 CDC20"^low)),
                                  expression(paste("G2/M italic(CDC20)"^low)),
                                  expression(paste("G2/M CDC20"^high))))+
...
auc_glotzer <- function() {
  ...
  plot_auc(dat, title=" ", comparisons = comparisons) + 
    scale_color_discrete(labels=c(expression(paste("G1 CDC20"^low)),
                                  expression(paste("G2/M CDC20"^low)),
                                  expression(paste("G2/M CDC20"^high))))+
    theme(legend.text.align = 0)
}
